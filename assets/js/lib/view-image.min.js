...
(function () {
    window.ViewImage = new function () {
        var b = this; this.target = "[view-image] img"; this.listener = function (a) { 
            if (!(a.ctrlKey || a.metaKey || a.shiftKey || a.altKey)) { 
                var d = String(b.target.split(",").map(function (g) { return g.trim() + ":not([no-view])" })), c = a.target.closest(d); 
                if (c) { 
                    var e = c.closest("[view-image]") || document.body; 
                    d = [].concat($jscomp.arrayFromIterable(e.querySelectorAll(d))).map(function (g) { return g.href || g.src }); 
        ...
        this.init = function(a) { 
            a && (b.target = a);
            // 增加对 .container 和 .container-memos 类的支持
            b.target = b.target + ", .container img, .container-memos img";  // 添加 .container 和 .container-memos img
